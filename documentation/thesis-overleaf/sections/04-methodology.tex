...
\textbf{Fourier time encoding/ Fourier feature mapping:}
...
\begin{figure}[htb]
  \centering
  % Stretched taller than its native aspect ratio (source is quite wide/short
  % and the axis labels/legend become unreadable at width=\textwidth alone).
  \includegraphics[width=\textwidth,height=11cm]{figures/fig-bc-dagger-schedule.png}
...
\textbf{Final Dataset Management Strategy:}
...
\newpage
\textbf{Observation Space (Original\footnote{The use of the term “original” is clarified later in this section.}) $\in \mathbb{R}^{44}$:}
...
\textbf{Noise and desynchronization:}
...
\textbf{Observation Space (Augmented) $\in \mathbb{R}^{98}$:}
...
\textbf{Critic state $\in \mathbb{R}^{76}$:}
...
\textbf{Behavior Cloning (BC) of multiple policies:}
