...
real_T Attachment_Point_Vectors[12] = { 0.5, 0.5, 0.2, -0.5, 0.5, -0.2, -0.5,
  -0.5, 0.15, 0.5, -0.5, -0.15 } ;
...
void Base_Model_step(void)
{
  ...
  for (i = 0; i < 3; i++) {
    ...
    for (i_0 = 0; i_0 < 3; i_0++) {
      ...
      tmp_c = _mm_add_pd(_mm_mul_pd(_mm_loadu_pd
        ...
        (Desired_Cable_Forces)), _mm_set_pd(rtb_RL_dot_0, localProduct));
      _mm_storeu_pd(&tmp_a[0], tmp_c);
  ...
  for (i = 0; i < 3; i++) {
    ...
    rtb_Sum3_idx_1 += rtb_Transpose[3 * i + 2] * Attachment_Point_Vectors[i];
  }

  Desired_Cable_Forces = 0.0;
  for (i = 0; i < 3; i++) {
    rtb_Transpose1[i] = ((Base_Model_DW.DiscreteTimeIntegrator2_DSTATE[i + 3] *
    ...
    Desired_Cable_Forces += rtb_RL_dot_0 * rtb_RL_dot_0;
  }
  ...
  for (i = 0; i < 3; i++) {
    ...
    tmp_c = _mm_add_pd(_mm_mul_pd(_mm_loadu_pd(&rtb_Transpose[3 * i]),
      _mm_set1_pd(Desired_Cable_Forces)), _mm_set_pd(rtb_MatrixMultiply5_idx_0,
      rtb_Sum3_idx_0));
    _mm_storeu_pd(&tmp_a[0], tmp_c);
    rtb_Sum3_idx_0 = tmp_a[0];
    rtb_MatrixMultiply5_idx_0 = tmp_a[1];
    rtb_Sum3_idx_1 += rtb_Transpose[3 * i + 2] * Desired_Cable_Forces;
  }
  ...
  for (i = 0; i < 3; i++) {
    rtb_Product7[i] = ((tmp[i + 3] * rtb_RL_dot_0 + tmp[i] * rtb_Gain) + tmp[i +
                       6] * rtb_MatrixMultiply5_idx_1) / localProduct *
      Cable_Resting_Length;
    Desired_Cable_Forces = Attachment_Point_Vectors[i + 9];
    tmp_c = _mm_add_pd(_mm_mul_pd(_mm_loadu_pd(&rtb_Transpose[3 * i]),
      _mm_set1_pd(Desired_Cable_Forces)), _mm_set_pd(rtb_MatrixMultiply5_idx_0,
      rtb_Sum3_idx_0));
    _mm_storeu_pd(&tmp_a[0], tmp_c);
  ...
  for (i = 0; i < 3; i++) {
    localProduct = Base_Model_DW.DiscreteTimeIntegrator2_DSTATE[i];
    Desired_Cable_Forces = localProduct * rtb_Sum3_idx_0;
    rtb_Transpose[3 * i] = localProduct;
    localProduct = Base_Model_DW.DiscreteTimeIntegrator2_DSTATE[i + 3];
    Desired_Cable_Forces += localProduct * rtb_MatrixMultiply5_idx_0;
    rtb_Transpose_tmp_0 = 3 * i + 1;
    rtb_Transpose[rtb_Transpose_tmp_0] = localProduct;
    localProduct = Base_Model_DW.DiscreteTimeIntegrator2_DSTATE[i + 6];
    rtb_Transpose_tmp = 3 * i + 2;
    rtb_Transpose[rtb_Transpose_tmp] = localProduct;
    rtb_Force_Sum[i] = (localProduct * rtb_Sum3_idx_1 + Desired_Cable_Forces) +
      Base_Model_DW.DiscreteTimeIntegrator_DSTATE[i];
    Desired_Cable_Forces = 0.0;
    rtb_RL_dot_0 = 0.0;
    localProduct = 0.0;
    for (i_0 = 0; i_0 < 3; i_0++) {
      ...
      tmp_c = _mm_add_pd(_mm_mul_pd(_mm_loadu_pd(&tmp[3 * i_0]), _mm_set1_pd
        (rtb_Gain)), _mm_set_pd(rtb_RL_dot_0, Desired_Cable_Forces));
      _mm_storeu_pd(&tmp_a[0], tmp_c);
      Desired_Cable_Forces = tmp_a[0];
      rtb_RL_dot_0 = tmp_a[1];
  ...
  rtb_Torque_Sum_0[2] = localProduct;
  rtb_Torque_Sum_0[1] = rtb_RL_dot_0;
  ...
  for (i = 0; i < 3; i++) {
    Desired_Cable_Forces = Base_Model_U.Desired_Cable_Forces[i + 3];
    tmp_3[0] += tmp_2[3 * i] * Desired_Cable_Forces;
    i_0 = 3 * i + 1;
    tmp_3[1] += tmp_2[i_0] * Desired_Cable_Forces;
    tmp_d = 3 * i + 2;
    tmp_3[2] += tmp_2[tmp_d] * Desired_Cable_Forces;
    Desired_Cable_Forces = 0.0;
    rtb_RL_dot_0 = 0.0;
    localProduct = 0.0;
    for (rtb_Transpose_tmp_0 = 0; rtb_Transpose_tmp_0 < 3; rtb_Transpose_tmp_0++)
    {
      ...
      tmp_c = _mm_add_pd(_mm_mul_pd(_mm_loadu_pd(&tmp[3 * rtb_Transpose_tmp_0]),
        _mm_set1_pd(rtb_Gain)), _mm_set_pd(rtb_RL_dot_0, Desired_Cable_Forces));
      _mm_storeu_pd(&tmp_a[0], tmp_c);
      Desired_Cable_Forces = tmp_a[0];
      rtb_RL_dot_0 = tmp_a[1];
    ...
    tmp_0[3 * i] = Desired_Cable_Forces;
    Desired_Cable_Forces = tmp_4[3 * i];
    ...
    tmp_4[3 * i] = Desired_Cable_Forces;
  }

  Desired_Cable_Forces = Base_Model_U.Desired_Cable_Forces[7];
  ...
  for (i = 0; i <= 0; i += 2) {
    ...
    tmp_9 = _mm_loadu_pd(&rtb_Torque_Sum_0[i]);
    tmp_7 = _mm_loadu_pd(&tmp_3[i]);
    _mm_storeu_pd(&tmp_5[i], _mm_add_pd(_mm_add_pd(_mm_add_pd(_mm_mul_pd(tmp_c,
      _mm_set1_pd(Desired_Cable_Forces)), _mm_mul_pd(tmp_b, _mm_set1_pd
      (localProduct))), _mm_mul_pd(tmp_8, _mm_set1_pd(rtb_RL_dot_0))),
      _mm_add_pd(tmp_9, tmp_7)));
    _mm_storeu_pd(&tmp_6[i], _mm_set1_pd(0.0));
  }

  for (i = 2; i < 3; i++) {
    tmp_5[i] = ((tmp_0[i + 3] * Desired_Cable_Forces + tmp_0[i] * localProduct)
                + tmp_0[i + 6] * rtb_RL_dot_0) + (rtb_Torque_Sum_0[i] + tmp_3[i]);
  }
  ...
  rtb_Torque_Sum_0[0] = ((rtb_Sum3_idx_1 * rtb_MatrixMultiply5_idx_2 -
    rtb_MatrixMultiply5_idx_1 * rtb_Sum3_idx_2) + (tmp_5[0] +
    Desired_Cable_Forces)) - Base_Model_DW.DiscreteTimeIntegrator3_DSTATE[0] *
    Load_Damping_Coef;
  Desired_Cable_Forces = 0.0;
  rtb_Torque_Sum_0[1] = ((rtb_MatrixMultiply5_idx_0 * rtb_Sum3_idx_2 -
    rtb_Sum3_idx_0 * rtb_MatrixMultiply5_idx_2) + (tmp_5[1] + rtb_RL_dot_0)) -
    Base_Model_DW.DiscreteTimeIntegrator3_DSTATE[1] * Load_Damping_Coef;
  rtb_RL_dot_0 = 0.0;
  rtb_Torque_Sum_0[2] = ((rtb_Sum3_idx_0 * rtb_MatrixMultiply5_idx_1 -
    rtb_MatrixMultiply5_idx_0 * rtb_Sum3_idx_1) + (tmp_5[2] + localProduct)) -
  ...
  for (i = 0; i < 3; i++) {
    tmp_c = _mm_add_pd(_mm_mul_pd(_mm_loadu_pd(&tmp[3 * i]), _mm_set1_pd
      (rtb_Torque_Sum_0[i])), _mm_set_pd(rtb_RL_dot_0, Desired_Cable_Forces));
    _mm_storeu_pd(&tmp_a[0], tmp_c);
    Desired_Cable_Forces = tmp_a[0];
    rtb_RL_dot_0 = tmp_a[1];
    localProduct += tmp[3 * i + 2] * rtb_Torque_Sum_0[i];
  }
  ...
  _mm_storeu_pd(&tmp_a[0], _mm_add_pd(_mm_mul_pd(tmp_c, _mm_set_pd
    (Base_Model_DW.UnitDelay_DSTATE[0], Desired_Cable_Forces)), _mm_set_pd
    (Base_Model_DW.DiscreteTimeIntegrator1_DSTATE_f[0],
     Base_Model_DW.DiscreteTimeIntegrator3_DSTATE[0])));
  ...
  _mm_storeu_pd(&tmp_a[0], _mm_add_pd(_mm_set_pd(rtb_Product7[0], rtb_Product6[0]),
    _mm_set_pd(rtb_Force_Sum[0], rtb_pH3_dot[0])));
  Base_Model_DW.UnitDelay_DSTATE_k[0] = tmp_a[0];
  Base_Model_DW.UnitDelay_DSTATE_c[0] = tmp_a[1];
  _mm_storeu_pd(&tmp_a[0], _mm_add_pd(_mm_mul_pd(tmp_c, _mm_set_pd
    (Base_Model_DW.UnitDelay_DSTATE[1], rtb_RL_dot_0)), _mm_set_pd
    (Base_Model_DW.DiscreteTimeIntegrator1_DSTATE_f[1],
     Base_Model_DW.DiscreteTimeIntegrator3_DSTATE[1])));
  ...
  _mm_storeu_pd(&tmp_a[0], _mm_add_pd(_mm_set_pd(rtb_Product7[1], rtb_Product6[1]),
    _mm_set_pd(rtb_Force_Sum[1], rtb_pH3_dot[1])));
  Base_Model_DW.UnitDelay_DSTATE_k[1] = tmp_a[0];
  Base_Model_DW.UnitDelay_DSTATE_c[1] = tmp_a[1];
  _mm_storeu_pd(&tmp_a[0], _mm_add_pd(_mm_mul_pd(tmp_c, _mm_set_pd
    (Base_Model_DW.UnitDelay_DSTATE[2], localProduct)), _mm_set_pd
    (Base_Model_DW.DiscreteTimeIntegrator1_DSTATE_f[2],
     Base_Model_DW.DiscreteTimeIntegrator3_DSTATE[2])));
  ...
  _mm_storeu_pd(&tmp_a[0], _mm_add_pd(_mm_set_pd(rtb_Product7[2], rtb_Product6[2]),
    _mm_set_pd(rtb_Force_Sum[2], rtb_pH3_dot[2])));
