...
def main(policy_path="il_actor_prdot.pt"):
    ...
    axes[2].set_xlabel("Time (s)"); fig.suptitle("Load tracking — pR_dot policy")
    ...
    plt.title("Drone velocity norms — pR_dot policy"); plt.legend(); plt.grid(True)
    ...
    plt.title("Drone XY trajectories — pR_dot policy"); plt.legend(); plt.grid(True); plt.axis("equal")
    ...
    ax4[-1].set_xlabel("Time (s)"); fig4.suptitle("Policy action (lambda) — pR_dot policy (closed loop)")
    ...
    print(f"pR_dot deploy:  mean track {err.mean():.4f} m   max {err.max():.4f} m")
    for i in range(N):
        print(f"  drone {i+1}: vel min {dvel[i].min():.3f}  mean {np.mean(dvel[i]):.3f}")
